...
class Test_SFT_Scenario_02_03:
    ...
    def initial(self):
        ...
        no_photo = subprocess.call(['adb', 'shell', 'ls', file_path], stdout=subprocess.DEVNULL,
                                   stderr=subprocess.STDOUT)
        if no_photo:
            logger("[Info] Start copying file ...")
            copy = subprocess.call(
                ['adb', 'push', path.join(path.dirname(__file__), 'test_material', file_name), material_path])
            if not copy:
