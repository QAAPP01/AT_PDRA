...
class AppiumXCUITestDriver(Borg, BaseDriver):
    def __init__(self, DriverConfig, AppConfig, mode="local", desired_caps={}):
        ...
        try:
            desired_caps = {}
            desired_caps.update(DriverConfig) if type(DriverConfig) == dict else logger(
                'Capability is invalid.(Not dict)'
            )
            # TODO: need to set parameter for cap
            desired_caps.update(AppConfig.iphd_cap) if type(
                AppConfig.iphd_cap) == dict else logger(
                'Capability is invalid.(Not dict)'
            )
            # TODO: need to set parameter for cap
    ...
    def install_app(self, path, package_name):
        ...
        try:
            ...
            try:
                ...
                logger(f'takes long time to install. Workarrount to continue installation {e}')
                time.sleep(1800)
...
# *******app control*********
    ...
    def remove_app(self, package_name):
        ...
        try:
            ...
            # create 'iOS-native app-settings" as master driver and start to trust our app
            desired_caps = {}
            desired_caps.update(DriverConfig) if type(
                DriverConfig) == dict else logger(
                'Capability is invalid.(Not dict)'
            )
            # TODO: need to set parameter for cap
            desired_caps.update(AppConfig.safari_cap) if type(
...
# ***Element control Functions***
    ...
    def tap(self, destination):
        ...
        try:
            ...
            # start to tap
            TA.tap(None, destination[0], destination[1], 1).perform()
            return True
    ...
    def get_element(self, locator, timeout=DEFAULT_TIMEOUT, poll_frequency=DEFAULT_POLL_TIME):
        ...
        for x in range(3):
            try:
                ...
                if str(exception) == 'an integer is required (got type NoneType)':
                    return None  # ignore the error (Nosuchelement exeception)
                elif str(exception) == "('Connection aborted.', ConnectionResetError(54, 'Connection reset by peer'))":
                    logger(f'[{__name__}][{funcname}]: Exception ({exception}), try again')
                    time.sleep(5)
                else:
                    logger('[{0}][{1}]: Exception ({2})'.format(__name__, funcname, exception))
                    break
